...
while($line = <F>) {
	...
	$paper_by_journal_country->{ $line[1] }->{ $line[4] }->{ $line[0] } = 1;
...
# print "writing num_paper_journal_country.tab...\n";
# open OUT, ">processed/num_paper_journal_country.tab" or die;
# print OUT "journal\tcountry\tpapers\n";
# foreach my $x (sort keys %$paper_by_journal_country) {
# 	foreach my $y (sort keys %{$paper_by_journal_country->{$x}}) {
# 		print OUT "$x\t$y\t".(scalar keys %{$paper_by_journal_country->{$x}->{$y}})."\n";
# 	}
# }
# close OUT;
...
my $output = `wc -l processed/citations.tab`;
$output =~s/^[^0-9]*(\d+).*$/$1/;
...
my $cite_by_journal = {};
my $cite_by_journal_and_country = {};
...
while($line = <F>) {
	...
	if(defined($pub->{$line[0]}) and defined($pub->{$line[1]})) {
		...
		$cite_by_journal_and_country->{ $pub->{$line[1]}->{journal_uid} }->{ $pub->{$line[1]}->{country} }->{ $pub->{$line[0]}->{country} } ++;
		...
		$paper_by_journal_country->{ $pub->{$line[1]}->{journal_uid} }->{ $pub->{$line[1]}->{country} }->{ $line[1] } = 1;
...
print "writing num_cite_journal_and_country.tab...\n";
open OUT, ">processed/num_cite_journal_country_country.tab" or die;
print OUT "journal_cited\tcountry_cited\tcountry_citing\tcitations\n";
foreach my $x (sort keys %$cite_by_journal_and_country) {
	foreach my $y (sort keys %{$cite_by_journal_and_country->{$x}}) {
		foreach my $z (sort keys %{$cite_by_journal_and_country->{$x}->{$y}}) {
			print OUT "$x\t$y\t$z\t$cite_by_journal_and_country->{$x}->{$y}->{$z}\n";
		}
	}
}
close OUT;
...
# print "writing num_paper_cited_journal_country.tab...\n";
# open OUT, ">processed/num_paper_cited_journal_country.tab" or die;
# print OUT "journal\tcountry\tpapers\n";
# foreach my $x (sort keys %$paper_by_journal_country) {
# 	foreach my $y (keys %{$paper_by_journal_country->{$x}}) {
# 		print OUT "$x\t$y\t".(scalar keys %{$paper_by_journal_country->{$x}->{$y}})."\n";
# 	}
# }
# close OUT;
