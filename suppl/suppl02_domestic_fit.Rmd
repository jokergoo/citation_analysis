...
Next we look at the expansion rate of $k_2/k_1$ (the difference of $k$ to the expected $k_\mathrm{exp}$. If there is an over-representation, $k$ can be decomposed to
...
**Scenario 1**: there is a weak positive relations between $p$ and $m$ in the log-scale, thus on the normal scale, we can approximate
...
speed ($\sim \alpha$, the second line in the equation above). Mixing these two speeds results the final speed being weak than quadratic.
...
In this case, in the expansion of the citation network, the "over-represented" part grows in a speed of $\sim \alpha \log \alpha$ (the second line in the equation above), but
